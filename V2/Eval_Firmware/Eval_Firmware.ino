...
read_write_O2 ();
//Button Handling
// Kalibrieren wenn GPIO 2 auf Masse gezogen wird wenn 3sekundedn gedrückt für 20 sekunden advertisen dann abschalten
if (!digitalRead(2)) {
  ble_i = ble_i + 1;
      if (!BLEconnected) {
        RFduinoBLE.begin();
...
if ((ble_adv_active == true) && (!BLEconnected)){
    ble_timeout = ble_timeout +1;
    //20 sekunden a 50ms = 400
    if (ble_timeout >= 400) {
    //RFduinoBLE.end();
    ble_active = false;
...
// wake up main loop() from INFINITE sleep
